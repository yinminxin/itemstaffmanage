...
$(function(){
  ...
  var $userfile = $('#uploadFile');  // the file you selected
  ...
  $("#uploadFile").change(function() {  // you can ues 'onchange' here to uplpad automatically after select a file
    $uploadedResult.html('');
    ...
    if (selectedFile) {
      ...
      var ramdomName = new Date().getTime() + $userfile.val().match(/\.?[^.\/]+$/);
      var keyVal = $key.val();
      $key.val(keyVal+ramdomName);
    ...
    $.ajax({
      ...
      success: function(res) {
        ...
        // $("#upresDiv").append('<input name="rPhotos" type="text" class="form-control"  placeholder="" value="'+domain+res.key+"#"+no+'">')
        $('#fileKey').val(domain + res.key);//填充值,提交后台表单写入数据库
